...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for i in contextWords:
        target = tokens[i]
        cost_iter, gradPred_iter, grad_iter = word2vecCostAndGradient(predicted, target, outputVectors, dataset)
        cost = cost + cost_iter
        gradIn[tokens[currentWord], :] += gradPred_iter  # we sum all the grad with respect to v_c
        gradOut += grad_iter
